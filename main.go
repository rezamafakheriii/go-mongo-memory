...
import (
	...
	"github.com/rezamafakheriii/go-mongo-memory/download"
	"github.com/rezamafakheriii/go-mongo-memory/monitor"
...
func StartWithOptions(ctx context.Context, version string, so ...ServerOption) (*Server, error) {
	...
	// you should store the binary file of mongodb in this location `{$homedir}/mongo`
	if runtime.GOOS == "windows" {
		homeDir, _ := os.UserHomeDir()
		binPath = path.Join(homeDir, "mongo/bin/mongod")
	} else {
		path, err := getOrDownloadBinPath(ctx, version)
		if err != nil {
	...
	// #TODO: currently this library does not support watcher for windows.
	// consider stopping mongodb by client.Stop() method in you code and always check if an instance is running in task manager
	if runtime.GOOS != "windows" {
	...
	if server.replSet != "" {
		...
		res := c.Database("admin").RunCommand(ctx, bson.D{{Key: "replSetInitiate", Value: replSetConfig}})
...
func (s *Server) Stop(ctx context.Context) {
	...
	// #TODO: in windows we should defer the removing temp and data files. because if we
	// remove the files immediately we get an error saying `another process uses this file...`
